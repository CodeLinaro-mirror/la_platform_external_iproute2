...
static int cbs_print_opt(const struct qdisc_util *qu, FILE *f, struct rtattr *opt)
{
	...
	fprintf(stderr, "hicredit %d ", qopt->hicredit);
	fprintf(stderr, "locredit %d ", qopt->locredit);
	fprintf(stderr, "sendslope %d ", qopt->sendslope);
	fprintf(stderr, "idleslope %d ", qopt->idleslope);
	fprintf(stderr, "offload %d ", qopt->offload);
